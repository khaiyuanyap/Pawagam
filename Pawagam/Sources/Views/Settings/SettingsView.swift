...
struct SettingsView: View {
    ...
    private var audioSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            sectionHeader(icon: "waveform", title: "Audio Options")

            VStack(spacing: 12) {
                ToggleOption(
                    icon: "speaker.wave.2",
                    title: "Audio",
                    subtitle: "Record audio alongside video",
                    isOn: $cameraManager.audioEnabled
                )
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    LazyVGrid(
                        ...
                        ForEach([44100.0, 48000.0, 96000.0], id: \.self) { sampleRate in
                            Button(action: { cameraManager.preferredAudioSampleRate = sampleRate }) {
                                Text(sampleRate == 44100.0 ? "44.1 kHz" : sampleRate == 48000.0 ? "48 kHz" : "96 kHz")
                                    .font(.system(size: 16, weight: .medium))
                                    .foregroundColor(
                                        cameraManager.preferredAudioSampleRate == sampleRate ? .black : .white
                                    )
                                    .frame(maxWidth: .infinity)
                                    .padding(.vertical, 10)
                                    .background(
                                        cameraManager.preferredAudioSampleRate == sampleRate ? Color.white : Color.white.opacity(0.2)
                                    )
                                    .cornerRadius(8)
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 8)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    LazyVGrid(
                        ...
                        ForEach([1, 2], id: \.self) { channels in
                            Button(action: { cameraManager.preferredAudioChannels = channels }) {
                                Text(channels == 1 ? "Mono" : "Stereo")
                                    .font(.system(size: 16, weight: .medium))
                                    .foregroundColor(
                                        cameraManager.preferredAudioChannels == channels ? .black : .white
                                    )
                                    .frame(maxWidth: .infinity)
                                    .padding(.vertical, 10)
                                    .background(
                                        cameraManager.preferredAudioChannels == channels ? Color.white : Color.white.opacity(0.2)
                                    )
                                    .cornerRadius(8)
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 8)
